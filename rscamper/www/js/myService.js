angular.module('App')
  ...
  .factory('detailLocationInfo',function ($rootScope) {
    return {
      getLocationInfo : function (no) {
        for (var i = 0; i < $rootScope.getScheduleLocation.length; i++) {
          if ($rootScope.getScheduleLocation[i].locationNo == no) {
            return $rootScope.getScheduleLocation[i];
          }
        }
        return null;
      }
